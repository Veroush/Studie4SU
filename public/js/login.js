...
const t = {
  nl: {
    ...
    errNameRequired: 'Voer je naam in (minimaal 2 tekens)', errEmailInvalid: 'Voer een geldig e-mailadres in',
  ...
  en: {
    ...
    errNameRequired: 'Please enter your name (at least 2 characters)', errEmailInvalid: 'Please enter a valid email address',
...
──────────────────────────────────────────────────────────── */
const dom = {
  heading: document.getElementById('form-heading'), subtitle: document.getElementById('form-subtitle'),
  nameField: document.getElementById('name-field'), nameInput: document.getElementById('name'),
  nameLabel: document.getElementById('name-label'), emailInput: document.getElementById('email'),
  emailLabel: document.getElementById('email-label'), passInput: document.getElementById('password'),
  passLabel: document.getElementById('password-label'), submitBtn: document.getElementById('submit-btn'),
  submitLabel: document.getElementById('submit-label'), submitArrow: document.getElementById('submit-arrow'),
  spinner: document.getElementById('spinner'), toggleText: document.getElementById('toggle-text'),
  toggleLink: document.getElementById('toggle-link'), apiBanner: document.getElementById('api-banner'),
  backLink: document.getElementById('back-link'), nameError: document.getElementById('name-error'),
...
document.getElementById('auth-form').addEventListener('submit', async function(e) {
  ...
  const payload = { email: dom.emailInput.value.trim(), password: dom.passInput.value };
  if (!isLoginMode) payload.name = dom.nameInput.value.trim();
...
dom.emailInput.addEventListener('blur', () => { if (dom.emailInput.value && !isValidEmail(dom.emailInput.value.trim())) showFieldError(dom.emailInput, dom.emailError, t[currentLang].errEmailInvalid); else clearFieldError(dom.emailInput, dom.emailError); });
dom.passInput.addEventListener('blur', () => { if (dom.passInput.value && dom.passInput.value.length < 6) showFieldError(dom.passInput, dom.passError, t[currentLang].errPasswordShort); else clearFieldError(dom.passInput, dom.passError); });
dom.nameInput.addEventListener('blur', () => { if (!isLoginMode && dom.nameInput.value && dom.nameInput.value.trim().length < 2) showFieldError(dom.nameInput, dom.nameError, t[currentLang].errNameRequired); else clearFieldError(dom.nameInput, dom.nameError); });
